...
| Method | Summary |
| - | - |
| [OpenCV](https://theclassytim.medium.com/using-image-processing-to-detect-text-8be34c677c11) | Perform a global threshold using Otsu's threshold method (This is if you desire the entire word. If you want individual letters, then use an adaptive threshold. A problem that can occur is for certain letters, each part can be determined as a different letter. For example, in a g the circle of the top and the bottom will be detected as 2 separate letters. This problem is fixed by setting the second argument in 'findContours()' to 'RETR_CCOMP'. Ignore contours with a parent. Create a bounding box for each contour. Another problem that can happen is words that are to close to each other which can be counted as a single word. The way this problem is fixed is to use a convoluted neural network to on each bounding box and determine if the letter in the box resembles an actual letter.|
